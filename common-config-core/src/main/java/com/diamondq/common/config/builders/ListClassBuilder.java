	...
	public <@NonNull T, @NonNull O> @Nullable ClassInfo<T, O> getClassInfo(Class<?> pClass, Class<O> pFinalClass,
		...
		if ((pType.getFactory().isPresent() == true) && (pType.getFactory().get().getValue().isPresent() == true)) {
			...
			try {
				...
				throw new I18NRuntimeException("bootstrap.factorynotfound", factoryClass,
					pType.getFactory().get().getConfigSource());
			}
		...
		for (ConfigClassBuilder ccb : pClassBuilders) {
			childResult = ccb.getClassInfo(pClass, pFinalClass, pType, pClassBuilders, pContext);
			if (childResult != null)
				break;
		}
